...
control 'CKV_AWS_346' do
  ...
  found = checkov_scan_regions(scan_regions).flat_map do |region|
    aws_network_firewall_firewall_policies(aws_region: region).names.to_a.map { |id| [id, region] }
  end
  ...
  in_scope = found.reject { |id, _r| checkov_exempt?(id: id, type: 'aws_networkfirewall_firewall_policy', rules: exempt) }

  if unusable.positive?
    describe "aws_networkfirewall_firewall_policy enumeration" do
  ...
  in_scope.each do |id, region|
    describe aws_network_firewall_firewall_policy(firewall_policy_name: id, aws_region: region) do
