...
#
# Rule:        CKV2_AWS_75 (checkov 3.3.16)
...
exempt       = (input('exempt_assets') || {})['CKV2_AWS_75'] || []

control 'CKV2_AWS_75' do
  title 'Ensure no open CORS policy'
  ...
  desc 'rationale', <<~RATIONALE
    A wildcard CORS origin lets any website in a victim's browser call the
    function URL with that browser's credentials and read the response.
  RATIONALE

  desc 'check', <<~CHECK
    Checkov looks for: Ensure no open CORS policy
  CHECK
  ...
  desc 'fix', <<~'FIX'
    ...
    Out of band — aws_lambda_function_url:
      ...
      aws lambda update-function-url-config --function-name app --cors AllowOrigins=https://app.example.com,AllowMethods=GET,AllowMethods=POST
  FIX

  tag checkov_id:            'CKV2_AWS_75'
  tag checkov_category:      'NETWORKING'
  tag checkov_version:       '3.3.16'
  tag checkov_kind:          'graph'
  tag tf_resources:          %w[aws_lambda_function aws_lambda_function_url]
  tag tf_docs:               'https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/lambda_function_url'
  tag nist:                  ['AC-4', 'SC-7', 'AC-3']
  tag nist_r4:               ['AC-4', 'SC-7', 'AC-3']
  tag cci:                   ['CCI-001414', 'CCI-002080']
  tag ksi:                   ['KSI-CNA-NDS']
  tag severity:              'medium'
  ...
  in_scope = enumerated
                        .reject { |a| a[:cors_allow_origins].nil? }
  ...
  in_scope.each do |asset|
    describe "aws_lambda_function_url #{asset[:id]} (#{asset[:account_id]}/#{asset[:region]})" do
      subject { asset[:cors_allow_origins] }
