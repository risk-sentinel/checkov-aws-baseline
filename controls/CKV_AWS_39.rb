...
control 'CKV_AWS_39' do
  ...
  desc 'fix', <<~'FIX'
    ...
    Note (aws_eks_cluster): Turning the public endpoint off without turning the
    private one on leaves the API server unreachable, so both are set together.
    Reaching a private-only endpoint needs a path into the VPC — a bastion, a
    VPN or Direct Connect — and anything that talks to the cluster from outside
    it, CI included, breaks the moment this lands unless that path exists first.
    Where the public endpoint has to stay, public_access_cidrs narrows it to
    known ranges; that is a mitigation and not a pass, and this check still
    reports it.
  FIX
  ...
  found = checkov_scan_regions(scan_regions).flat_map do |region|
    ids, found_problems = checkov_enumerate(
      aws_eks_clusters(aws_region: region), :names, exclude: { statuses: ['DELETING'] }
    )
