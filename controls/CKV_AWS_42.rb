...
control 'CKV_AWS_42' do
  ...
  found = checkov_scan_regions(scan_regions).flat_map do |region|
    aws_efs_file_systems(aws_region: region).names.to_a.map { |id| [id, region] }
  end
  ...
  in_scope = found.reject { |id, _r| checkov_exempt?(id: id, type: 'aws_efs_file_system', rules: exempt) }
