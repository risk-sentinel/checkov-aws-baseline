...
exempt       = (input('exempt_assets') || {})['CKV_AWS_7'] || []

control 'CKV_AWS_7' do
  ...
  desc 'rationale', <<~RATIONALE
    Ensure rotation for customer created CMKs is enabled. Anchors derived
    from the check's encryption category, not reviewed control by control.
  RATIONALE

  desc 'check', <<~CHECK
    Checkov looks for: aws_kms_key: enable_key_rotation is True
  CHECK

  desc 'fix', <<~'FIX'
    See https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/kms_key#enable-key-rotation
  FIX

  tag checkov_id:            'CKV_AWS_7'
  tag checkov_category:      'ENCRYPTION'
  tag checkov_version:       '3.3.16'
  tag checkov_kind:          'value'
  tag tf_resources:          %w[aws_kms_key]
  tag tf_docs:               'https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/kms_key#enable-key-rotation'
  ...
  found = checkov_scan_regions(scan_regions).flat_map do |region|
    aws_kms_keys(aws_region: region).key_ids.to_a.map { |id| [id, region] }
  end
  ...
  in_scope = found.reject { |id, _r| checkov_exempt?(id: id, type: 'aws_kms_key', rules: exempt) }
  ...
  in_scope.each do |id, region|
    describe aws_kms_key(key_id: id, aws_region: region) do
      its('enable_key_rotation') { should eq true }
    end
  end
end
