...
#
# Rule:        CKV2_AWS_74 (checkov 3.3.16)
...
exempt       = (input('exempt_assets') || {})['CKV2_AWS_74'] || []

control 'CKV2_AWS_74' do
  title 'Ensure AWS Load Balancers use strong ciphers'
  ...
  desc 'rationale', <<~RATIONALE
    The named legacy policies carry CBC and RC4-era suites and 3DES. They
    exist only for clients that should no longer be reaching a FedRAMP
    boundary.
  RATIONALE

  desc 'check', <<~CHECK
    Checkov looks for: Ensure AWS Load Balancers use strong ciphers
  CHECK
  ...
  desc 'fix', <<~'FIX'
    ...
    Note (aws_lb_listener): ELBSecurityPolicy-2016-08 is the ELBv2 default, so a
    listener that never set ssl_policy explicitly is on a weak policy.
  FIX

  tag checkov_id:            'CKV2_AWS_74'
  ...
  tag nist:                  ['SC-8', 'SC-13', 'SC-8 (1)']
  tag nist_r4:               ['SC-8', 'SC-13']
  ...
  in_scope.each do |asset|
    describe "aws_lb_listener #{asset[:id]} (#{asset[:account_id]}/#{asset[:region]})" do
      ...
      it { should_not be_in ['ELBSecurityPolicy-2016-08', 'ELBSecurityPolicy-2015-05', 'ELBSecurityPolicy-TLS-1-0-2015-04', 'ELBSecurityPolicy-TLS-1-1-2017-01', 'ELBSecurityPolicy-2015-03', 'ELBSecurityPolicy-2015-02', 'ELBSecurityPolicy-2014-10', 'ELBSecurityPolicy-Default', 'ELBSecurityPolicy-2014-01'] }
    end
  end
end
