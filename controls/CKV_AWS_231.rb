...
#
# Rule:        CKV_AWS_231 (checkov 3.3.16)
...
exempt       = (input('exempt_assets') || {})['CKV_AWS_231'] || []
...
element_conditions = [
  ...
  # port_range.from <= 3389 (or absent)
  { paths: ['port_range.from'],
    when_absent: true,
    test: ->(v) { v <= 3389 } },
  # port_range.to >= 3389 (or absent)
  { paths: ['port_range.to'],
    when_absent: true,
    test: ->(v) { v >= 3389 } },
].freeze

control 'CKV_AWS_231' do
  title 'Ensure no NACL allow ingress from 0.0.0.0:0 to port 3389'
  ...
  desc 'rationale', <<~RATIONALE
    3389 is RDP, an interactive administrative session. Exposed to 0.0.0.0/0
    it is credential-stuffed continuously and is the single most common
    initial access vector for ransomware in cloud estates. Remote
    administrative access is supposed to arrive over a managed path —
    Session Manager, a bastion, a VPN — and a subnet-wide ACL entry silently
    makes every instance behind it directly reachable instead.
  RATIONALE

  desc 'check', <<~CHECK
    Checkov looks for: a network ACL with no ingress allow entry that
    reaches TCP port 3389 from 0.0.0.0/0 or ::/0
  CHECK
  ...
  desc 'fix', <<~'FIX'
    ...
    Out of band — aws_network_acl:
      ...
      aws ec2 replace-network-acl-entry --network-acl-id <acl-id> --rule-number <n> --protocol tcp --rule-action allow --ingress --cidr-block 10.0.0.0/16 --port-range From=3389,To=3389

    Note (aws_network_acl): Narrowing the CIDR to the management network is the
    minimum. The better answer is to remove the entry entirely and reach Windows
    hosts through Systems Manager Session Manager, which needs no inbound rule
    at any layer and produces a session log; that also removes the bastion whose
    own ingress rule would otherwise inherit this finding. Network ACLs are
    stateless, so keep the ephemeral egress range when editing.
  FIX

  tag checkov_id:            'CKV_AWS_231'
  ...
  tag nist:                  ['SC-7', 'SC-7 (5)', 'AC-4', 'AC-17']
  tag nist_r4:               ['SC-7', 'SC-7 (5)', 'AC-4', 'AC-17']
  ...
  in_scope.each do |asset|
    describe "aws_network_acl #{asset[:id]} (#{asset[:account_id]}/#{asset[:region]})" do
      ...
      it { should satisfy("have no element where egress == False and rule_action == allow and protocol is one of [6, -1] and (cidr_block or ipv_6_cidr_block) is one of [0.0.0.0/0, ::/0] and port_range.from <= 3389 (or absent) and port_range.to >= 3389 (or absent)") { |v| ::CheckovCollection.none_of?(v, element_conditions) } }
    end
  end
end
