...
control 'CKV_AWS_207' do
  ...
  found = checkov_scan_regions(scan_regions).flat_map do |region|
    aws_mq_brokers(aws_region: region).broker_ids.to_a.map { |id| [id, region] }
  end
  ...
  in_scope = found.reject { |id, _r| checkov_exempt?(id: id, type: 'aws_mq_broker', rules: exempt) }

  if unusable.positive?
    describe "aws_mq_broker enumeration" do
  ...
  in_scope.each do |id, region|
    describe aws_mq_broker(broker_id: id, aws_region: region) do
