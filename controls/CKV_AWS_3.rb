...
#
# Rule:        CKV_AWS_3 (checkov 3.3.16)
# Applies to:  aws_ebs_volume
# Read with:   aws_ebs_volumes -> aws_ebs_volume (stock inspec-aws, no custom reader)
#
# The rule id is the identity: file name, control id and `tag checkov_id` all
# carry it, and tools/lint_catalog_drift.py asserts the three agree.

scan_regions = input('scan_regions')
exempt       = (input('exempt_assets') || {})['CKV_AWS_3'] || []

control 'CKV_AWS_3' do
  title 'Ensure all data stored in the EBS is securely encrypted'

  desc <<~DESC
    Checkov asserts this against Terraform. This profile asserts it against
    the aws_ebs_volume resources that actually exist, read through the stock
    inspec-aws aws_ebs_volume resource.
  DESC

  desc 'rationale', <<~RATIONALE
    Ensure all data stored in the EBS is securely encrypted. Anchors derived
    from the check's encryption category, not reviewed control by control.
  RATIONALE

  desc 'check', <<~CHECK
    Checkov looks for: aws_ebs_volume: encrypted is True
  CHECK

  desc 'fix', <<~'FIX'
    See https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/ebs_volume#encrypted
  FIX

  tag checkov_id:            'CKV_AWS_3'
  tag checkov_category:      'ENCRYPTION'
  tag checkov_version:       '3.3.16'
  tag checkov_kind:          'value'
  tag tf_resources:          %w[aws_ebs_volume]
  tag tf_docs:               'https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/ebs_volume#encrypted'
  ...
  found = checkov_scan_regions(scan_regions).flat_map do |region|
    ids, found_problems = checkov_enumerate(
      aws_ebs_volumes(aws_region: region), :volume_ids
    )
  ...
  in_scope = found.reject { |id, _r| checkov_exempt?(id: id, type: 'aws_ebs_volume', rules: exempt) }
  ...
  in_scope.each do |id, region|
    describe aws_ebs_volume(volume_id: id, aws_region: region) do
