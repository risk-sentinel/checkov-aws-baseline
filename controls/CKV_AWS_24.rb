...
#
# Rule:        CKV_AWS_24 (checkov 3.3.16)
...
exempt       = (input('exempt_assets') || {})['CKV_AWS_24'] || []
...
element_conditions = [
  # from_port <= 22 (or absent)
  { paths: ['from_port'],
    when_absent: true,
    test: ->(v) { v <= 22 } },
  # to_port >= 22 (or absent)
  { paths: ['to_port'],
    when_absent: true,
    test: ->(v) { v >= 22 } },
...
control 'CKV_AWS_24' do
  ...
  desc 'rationale', <<~RATIONALE
    A security group is the last boundary in front of the instance, and SSH
    on port 22 is an administrative channel. Opened to 0.0.0.0/0 it is
  ...
  desc 'check', <<~CHECK
    ...
    from_port..to_port span covers 22 — or whose protocol is -1, which
  ...
  desc 'fix', <<~'FIX'
    Terraform — aws_security_group:
      ...
      resource "aws_security_group" "example" {
        ...
        # SSH arrives from the bastion's security group, never from a CIDR.
    ...
    Out of band — aws_security_group:
      ...
      aws ec2 revoke-security-group-ingress --group-id <sg-id> --ip-permissions IpProtocol=tcp,FromPort=22,ToPort=22,Ipv6Ranges=[{CidrIpv6=::/0}]
    ...
    port range merely SPANS 22 — 0-65535, or an IpProtocol of "-1", which AWS
    stores with no ports at all — has to be revoked with the range it was
    created with, not with --port 22.
  ...
  in_scope.each do |asset|
    describe "aws_security_group #{asset[:id]} (#{asset[:account_id]}/#{asset[:region]})" do
      ...
      it { should satisfy("have no element where from_port <= 22 (or absent) and to_port >= 22 (or absent) and (ip_ranges.cidr_ip or ipv_6_ranges.cidr_ipv_6) is one of [0.0.0.0/0, ::/0, 0000:0000:0000:0000:0000:0000:0000:0000/0]") { |v| ::CheckovCollection.none_of?(v, element_conditions) } }
    end
  end
end
