...
specs.each do |type, spec|
  ...
  required = operation.input.shape.required.to_a
  unless required.empty?
    errors << "#{type}: #{spec['list']} REQUIRES #{required.join(', ')}, but the reader " \
              "calls it with no arguments — every region would raise into unreadable_regions"
  end
