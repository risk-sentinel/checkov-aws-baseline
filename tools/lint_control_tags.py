...
# %w[a b] and %i[a b] are array literals of strings/symbols. The AST sees an
# `array` of `str`/`sym` nodes and reads them fine -- verified by serialising a
# profile that uses one and finding the values intact, not by assuming.
PERCENT_LITERAL = re.compile(r"^%[wi][\[({<][^\])}>]*[\])}>]$")


def non_literal_parts(raw):
    """The parts of a tag value InSpec cannot read from the AST."""
    if PERCENT_LITERAL.match(raw):
        return []
